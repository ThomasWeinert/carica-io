  ...
  class Device {
    ...
    public function __construct($string) {
      if (substr(PHP_OS, 0, 3) === "WIN") {
        $pattern = '(^COM\d+:$)';
        $command = sprintf(
            'mode %s BAUD=57600 PARITY=N data=8 stop=1 xon=off', strtolower($this->_device)
        );
      } elseif (substr(PHP_OS, 0, 6) === "Darwin") {
        $pattern = '(^COM\d+:$)';
        $command = sprintf('stty -F %s', $this->_device);
      } elseif (substr(PHP_OS, 0, 5) === "Linux") {
        $pattern = '(^/dev/tty\w+\d+$)';
        $prepare = sprintf('stty -F %s', $this->_device);
      } else {
        throw new LogicException(sprintf('Unsupport OS: "%s".', PHP_OS));
      }
      if (!preg_match($device, $pattern)) {
